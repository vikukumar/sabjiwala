...
function FacebookCallbackHandler() {
  ...
  const [status, setStatus] = useState("Exchanging Facebook credentials...");
  ...
  useEffect(() => {
    const exchangeCode = async () => {
      ...
      if (!code) {
        setError("Authorization code is missing from Facebook redirect.");
        return;
      }
      ...
      try {
        ...
        if (res.success && res.meta) {
          ...
          setError(res.message || "Failed to log in with Facebook.");
        }
      } catch (err: any) {
        setError(err.response?.data?.detail || err.message || "Network error during Facebook callback.");
      }
    };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#090d10] text-slate-800 dark:text-slate-100 flex items-center justify-center p-6 font-sans">
      <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 text-center space-y-4 shadow-xl">
        ...
        <h3 className="text-lg font-black text-slate-900 dark:text-white">Logging in via Facebook</h3>
        <p className="text-xs text-slate-500 dark:text-slate-455">{status}</p>
      </div>
    </div>
  );
}
