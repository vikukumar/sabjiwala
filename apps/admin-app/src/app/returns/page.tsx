...
import { 
  FileText, Check, X, Loader2, ArrowUpRight, 
  ExternalLink, Eye, ChevronDown, ChevronUp, Image as ImageIcon 
} from "lucide-react";
...
export default function AdminReturnsPage() {
  ...
  return (
    <AdminLayout title="Returns & Refunds Manager">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-sm">
          <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">Review return requests, proof images, and approve refund claims.</p>
            </div>
          </div>
          ...
          {isLoading ? (
            <div className="py-24 flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Loading Requests...</p>
            </div>
          ) : returnRequests.length === 0 ? (
            <div className="py-24 text-center">
              <span className="text-4xl">📦</span>
              <h4 className="text-sm font-black text-slate-800 dark:text-white mt-3">No return requests found</h4>
              <p className="text-xs text-slate-500 mt-1">Customers haven't submitted any product returns yet.</p>
            </div>
          ) : (
                  ...
                  <div key={req.id} className="transition-all hover:bg-slate-50/50 dark:hover:bg-slate-800/10">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-850 flex items-center justify-center font-black text-xs text-slate-650 dark:text-slate-400">
                          #{req.order_number.slice(-6)}
                        </div>
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="font-black text-xs">{req.customer_name}</span>
                            <span className="text-[10px] text-slate-400">({req.customer_phone})</span>
                          </div>
                      ...
                      <div className="flex items-center gap-6">
                        <div className="text-right">
                          <p className="text-[9px] text-slate-400 uppercase font-black tracking-wider">Refund Claim</p>
                        ...
                        <button className="text-slate-400 hover:text-slate-600 p-1 bg-transparent border-0">
                    ...
                    {isExpanded && (
                      <div className="px-5 pb-6 pt-1 bg-slate-50/30 dark:bg-slate-900/30 border-t border-slate-100 dark:border-slate-800/60 animate-fade-in space-y-4">
                        {/* Overall Reason */}
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {req.return_items?.map((item, idx) => (
                              <div key={idx} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-3 flex items-center justify-between gap-3 shadow-sm">
                                <div className="flex items-center gap-2.5">
                                  ...
                                  <div>
                                    ...
                                    <span className="text-[10px] text-slate-400 mt-0.5 block font-bold">Qty: {item.quantity} × ₹{item.price}</span>
                                  </div>
                                </div>
                                <div className="text-right">
