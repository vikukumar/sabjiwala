...
export default function DeliveryRegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post("/auth/register", {
        ...
        license_number: regLicenseNumber
      });
  ...
  return (
    <div className="min-h-screen bg-slate-55 dark:bg-[#090d10] text-slate-800 dark:text-slate-100 flex flex-col justify-between transition-colors duration-200 antialiased font-sans py-6">
      {/* Header */}
      <header className="max-w-6xl w-full mx-auto px-4 h-16 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="bg-emerald-100 dark:bg-emerald-950/50 text-emerald-800 dark:text-emerald-300 text-[10px] font-bold px-2 py-0.5 rounded-full">Rider Registration</span>
        </div>
        <button
          onClick={toggleTheme}
          className="p-2 rounded-full bg-white dark:bg-slate-900 text-slate-655 dark:text-slate-350 hover:scale-105 active:scale-95 transition-all shadow-sm border border-slate-205 dark:border-slate-800 hover:border-emerald-300 dark:hover:border-emerald-500/30"
        >
          {theme === "light" ? "🍆" : "🍋"}
        </button>
      </header>
      ...
      <main className="flex-1 flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-205 dark:border-slate-800/80 rounded-3xl shadow-xl p-6 md:p-8 space-y-6 relative overflow-hidden my-4">
          ...
          <div className="text-center space-y-2">
            <Truck className="w-10 h-10 text-emerald-600 mx-auto" />
            <h2 className="text-xl md:text-2xl font-black tracking-tight text-slate-900 dark:text-white">
              {otpMode === "register" ? "Deliver with Us" : "Verify Your Account"}
            </h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              {otpMode === "register" ? "Register as a delivery partner and start earning" : "Enter OTP sent to " + regEmail}
            </p>
          </div>
          ...
          {otpMode === "register" ? (
            <form onSubmit={handleRegister} className="space-y-3.5">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <div className="flex items-center border border-slate-205 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950 rounded-xl px-3.5 py-2.5 focus-within:border-emerald-500 transition-all">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center border border-slate-205 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-955 rounded-xl px-3.5 py-2.5 focus-within:border-emerald-500 transition-all">
                    <User className="w-4 h-4 text-slate-455 mr-2" />
                    <input
                      type="text"
                      placeholder="Sharma"
              ...
              <div className="space-y-3 p-4 rounded-2xl border border-blue-500/20 bg-blue-50/10 dark:bg-blue-955/10 text-left">
                <p className="text-xs font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wider mb-2">Vehicle Settings</p>
                
                <div className="space-y-1.5">
                  <label className="text-[10px] font-semibold text-slate-700 dark:text-slate-350">Vehicle Type</label>
                  <select
                    value={regVehicleType}
                    onChange={(e) => setRegVehicleType(e.target.value)}
                    className="w-full px-3.5 py-2.5 text-xs rounded-xl border border-slate-205 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 outline-none focus:border-emerald-500 transition-all"
                  >
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 tracking-wider">Vehicle Plate Number</label>
                  <div className="flex items-center border border-slate-205 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950 rounded-xl px-3.5 py-2.5 focus-within:border-emerald-500 transition-all">
                    <input
                      type="text"
                      required
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 tracking-wider">Driver License Number</label>
                  <div className="flex items-center border border-slate-205 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-955 rounded-xl px-3.5 py-2.5 focus-within:border-emerald-500 transition-all">
                    <input
                      type="text"
                      required
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <div className="flex items-center border border-slate-205 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950 rounded-2xl px-4 py-3 focus-within:border-emerald-500 transition-all">
