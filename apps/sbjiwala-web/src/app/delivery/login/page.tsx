...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined" && localStorage.getItem("sw_access_token")) {
      ...
      const isStandaloneVendor = !isUnified && (process.env.NEXT_PUBLIC_APP_MODE === "vendor" || window.location.port === "3001" || window.location.host.includes("vendor"));
      const isStandaloneDelivery = !isUnified && (process.env.NEXT_PUBLIC_APP_MODE === "delivery" || window.location.port === "3002" || window.location.host.includes("delivery") || window.location.host === "localhost");
      const isStandaloneAdmin = !isUnified && (process.env.NEXT_PUBLIC_APP_MODE === "admin" || window.location.port === "3003" || window.location.host.includes("admin"));
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success && res.meta) {
        ...
        handleSuccessfulLoginRedirect(res.meta.role || "delivery_boy");
      } else {
  ...
  const handlePasswordLogin = async (e: React.FormEvent) => {
    ...
    setSubmitLoading(true);
    try {
      const encrypted = await encryptPayload({ email, password, role: "delivery_boy" });
        ...
        <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 rounded-3xl shadow-xl p-6 md:p-8 space-y-6 relative overflow-hidden">
          ...
          <div className="text-center space-y-2 relative">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Log in to manage your delivery assignments and track orders
            </p>
          </div>
          ...
          <div className="space-y-4">
              ...
              !isOtpSent ? (
                <form onSubmit={handleSendOtp} className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <div className="flex items-center border border-slate-200 dark:border-slate-800 bg-slate-55/50 dark:bg-slate-955 rounded-2xl px-4 py-3 focus-within:border-emerald-500 transition-all">
            ...
            <div className="text-center text-xs font-semibold text-slate-550 dark:text-slate-400 pt-2">
              ...
              <Link href={process.env.NEXT_PUBLIC_APP_MODE === "unified" ? "/delivery/register" : "/register"} className="text-emerald-650 dark:text-emerald-400 hover:underline font-extrabold">
                Register as a Delivery Partner here
              </Link>
            </div>
            ...
            {!isNativeApp && (
              <div className="flex justify-between items-center text-[10px] font-semibold text-slate-400 dark:text-slate-500 pt-4 border-t border-slate-100 dark:border-slate-800 mt-2">
                <a href={process.env.NEXT_PUBLIC_APP_MODE === "unified" ? "/app/login" : "http://localhost:3000/login"} className="hover:text-emerald-650 dark:hover:text-emerald-400 flex items-center gap-1">
                  ← Customer Portal
                </a>
