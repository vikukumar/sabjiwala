...
export default function TicketDetailClient() {
  ...
  if (!id) return <div className="text-center py-20 text-slate-500 font-sans">Ticket ID is missing</div>;

  return (
    <div className="max-w-2xl mx-auto flex flex-col font-sans" style={{ height: "calc(100vh - 64px)" }}>
      {/* Header */}
      <div className="px-4 py-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-slate-550 dark:text-slate-400 capitalize">{ticket?.category?.replace(/_/g, " ")}</p>
          </div>
        ...
        {(ticket?.messages || []).map((msg: any, i: number) => {
          ...
          return (
            <div key={i} className={`flex gap-2 ${isAgent ? "" : "flex-row-reverse"}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${isAgent ? "bg-blue-100 dark:bg-blue-955/50" : "bg-emerald-100 dark:bg-emerald-955/50"}`}>
              ...
              <div className={`rounded-2xl p-3 max-w-[80%] ${isAgent ? "bg-slate-100 dark:bg-slate-800 rounded-tl-none" : "bg-emerald-50 dark:bg-emerald-955/20 border border-emerald-100 dark:border-emerald-900/30 rounded-tr-none"}`}>
