...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      if (accessToken) {
        ...
        const role = getStoredUserType() || "admin";
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success && res.meta) {
        ...
        handleSuccessfulLoginRedirect(res.meta.role || "admin");
      } else {
  ...
  const handleSocialLogin = (provider: "google" | "facebook") => {
    ...
    const scheme = "in.sbjiwala.admin://login";
              ...
              {(socialSettings.google_client_id || socialSettings.facebook_client_id) && (
                <div className="space-y-4 mt-6 pt-4 border-t border-slate-200 dark:border-slate-800">
                  <div className="relative flex py-1 items-center">
                    ...
                    <span className="flex-shrink mx-4 text-slate-405 text-[10px] font-bold uppercase tracking-wider">Or Connect With</span>
                    <div className="flex-grow border-t border-slate-200 dark:border-slate-800"></div>
                  </div>
