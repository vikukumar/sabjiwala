        ...
        {mainNavItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={resolvedHref}
              data-longpress={item.href === "/profile" ? "profile" : undefined}
              onClick={(e) => {
                if (isProtectedRoute(resolvedHref) && !localStorage.getItem("sw_access_token")) {
                  e.preventDefault();
                  router.push(`${resolveLink("/login")}?redirect=${encodeURIComponent(resolvedHref)}`);
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useWebSocket((message: any) => {
    ...
    if (
      ...
      const title = message.data?.title || (message.type === "order_status_update" ? "Order Update" : "Notification Received");
      const body = message.data?.body || message.data?.message || `Status changed to ${message.data?.status || ""}`;
            ...
            <div className="flex flex-col gap-0.5 mt-1">
                  ...
                  <button
                    onClick={() => {
                      if (contextMenu.targetId) {
                        router.push(resolveLink(`/orders/${contextMenu.targetId}`));
                      }
                    }}
              ...
              {contextMenu.type === "profile" && (
                <>
                  ...
                  <button
                    onClick={() => router.push(resolveLink("/addresses"))}
                    className="flex items-center gap-2 px-3 py-2 hover:bg-slate-50 dark:hover:bg-slate-850 rounded-lg text-left cursor-pointer border-0 bg-transparent text-slate-800 dark:text-slate-200"
                  >
                  ...
                  <button
                    onClick={() => router.push(resolveLink("/wallet"))}
                    className="flex items-center gap-2 px-3 py-2 hover:bg-slate-50 dark:hover:bg-slate-850 rounded-lg text-left cursor-pointer border-0 bg-transparent text-slate-800 dark:text-slate-200"
                  >
