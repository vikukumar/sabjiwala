...
export default function ProductDetailClient() {
  ...
  const cartItem = cartData?.items?.find((i: any) => i.product_id === id);
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="fixed bottom-16 md:bottom-0 left-0 right-0 md:left-64 bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm border-t border-slate-200 dark:border-slate-800 px-4 py-3 z-30">
        <div className="max-w-2xl mx-auto flex items-center gap-4">
            ...
            {cartItem ? (
              <div className="flex items-center justify-center gap-4 bg-emerald-600 text-white rounded-2xl py-3 px-6 shadow-lg">
                <button
                  ...
                  className="w-8 h-8 flex items-center justify-center bg-emerald-700 hover:bg-emerald-800 rounded-xl transition-colors"
                >
          ...
          {cartItem && (
            <Link href="/cart">
              <Button variant="secondary" size="lg" rightIcon={<ArrowRight className="w-4 h-4" />} className="rounded-2xl flex-shrink-0">
                Go to Cart
              </Button>
            </Link>
          )}
