...
function LoginPageContent() {
  ...
  const handleSocialLogin = (provider: "google" | "facebook") => {
    ...
    const scheme = "in.sbjiwala.customer://login";
  ...
  // Check existing redirect session or verify magic tokens or OAuth tokens
  useEffect(() => {
    ...
    if (accessToken) {
      ...
      syncGuestCart(queryClient).then(() => {
        const userType = getStoredUserType() || "customer";
  ...
  }, [searchParams, queryClient, success]);
      ...
      <div className="flex-1 flex items-center justify-center p-6 bg-slate-50 dark:bg-[#090d10]">
        <div className="w-full max-w-md space-y-6">
          ...
          <div className="card p-8 shadow-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl">
            ...
            <div className="text-center text-xs font-semibold text-slate-555 dark:text-slate-450 mt-6">
              Don't have an account?{" "}
              <Link href={resolveLink("/register")} className="text-emerald-655 dark:text-emerald-450 hover:underline font-extrabold">
                Register here
              </Link>
            </div>
